package com.github.xkk.android_uikit.main.hotFix;
...
public class HotFixUtil {
    ...
    private final String TAG = "Android_UIKIT";
    ...
    public void startFix() throws IllegalAccessException, NoSuchFieldException, ClassNotFoundException {
        ...
        File sourceFile  = Environment.getExternalStorageDirectory();
        ...
        File optFile = BaseApplication.getContext().getDir(OPTIMIZE_DIR, Context.MODE_PRIVATE);
        if (!optFile.exists()) {
            optFile.mkdir();
        }
        ...
        for (int i = 0; i < listFiles.length; i++) {//遍历查找文件中patch开头, .dex .jar .apk结尾的文件
            ...
            if (file.getName().startsWith("Test") && file.getName().endsWith(DEX_SUFFIX)//这里我默认的补丁文件名是patch
        ...
        ClassLoader pathClassLoader = BaseApplication.getContext().getClassLoader();//拿到系统默认的PathClassLoader加载器
        DexClassLoader dexClassLoader = new DexClassLoader(dexPath, optPath, null, BaseApplication.getContext().getClassLoader());//加载我们自己的补丁dex
